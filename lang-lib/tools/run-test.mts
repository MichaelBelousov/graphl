...
interface UserFuncInput {
  type: GraphlType;
}

interface UserFuncOutput {
  type: GraphlType;
}
...
function makeCallUserFunc<F>(inputs: UserFuncInput[], outputs: UserFuncOutput[]): [string, Function] {
    ...
    return [
        ...
        (funcId: number, abiParams: any[]) => {
            ...
            const paramsToJs = (params: any[]): any[] => {
                ...
                for (let i = 0; i < params.length; ++i) {
                    ...
                    if (paramType === GraphlTypes.string) {
                        ...
                        while (true) {
                            const bytesWrittenCount = wasm.instance.exports.__graphl_host_copy(paramWasmVal, offset);
                            if (bytesWrittenCount === 0) break;
                            offset += bytesWrittenCount;
                            const chunk = new Uint8Array(wasm.instance.exports.memory.buffer, TRANSFER_BUF_PTR, bytesWrittenCount);
                            chunks.push(chunk);
            ...
            if (firstArgIsReturnPtr && outputs[0].type.kind === "struct") {
                ...
                const resultView = new DataView(wasm.instance.exports.memory.buffer, returnPtr, outputs[0].type.size);
...
console.log({
    env: Object.fromEntries([
        makeCallUserFunc([], [{ type: GraphlTypes.vec3 }]),
        ["test", () => {}],
    ]),
});
...
const result = wasm.instance.exports[functionToCall]();
console.log(result);
