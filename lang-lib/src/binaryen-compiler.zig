...
pub fn compileFunc(wasm_module: binaryen.Module, sexp: *const Sexp) void {
    ...
    const wasm_func = binaryen.AddFunction.create(wasm_module, func_name, params, results, null, 0, add);
}

pub fn compileTypeOf(wasm_module: binaryen.Module, sexp: *const Sexp) void {
    _ = wasm_module;
    std.debug.assert(sexp.value == .list);
    std.debug.assert(sexp.value.list.items.len >= 1);
    std.debug.assert(sexp.value.list.items[0].value == .symbol);
...
fn compileInternal(sexp: *Sexp, wasm_module: binaryen.Module, _in_diagnostic: *Diagnostic) void {

}

pub fn compile(sexp: *const Sexp, _in_diagnostic: ?*Diagnostic) void {
    const wasm_module = binaryen.Module.create();
    ...
    const x = c.binaryen.localGet(wasm_module, 0, c.BinaryenTypeInt32());
    const y = binaryen.localGet(wasm_module, 1, c.BinaryenTypeInt32());
    const add = binaryen.Binary(wasm_module, c.BinaryenAddInt32(), x, y);

    // Create the add function
    // Note: no additional local variables
    // Note: no basic blocks here, we are an AST. The function body is just an
    // expression node.
    const adder = binaryen.addFunction(wasm_module, "adder", params, results, null, 0, add);
    _ = adder;

    // Print it out
    binaryen.Module.print(wasm_module);

    // Clean up the module, which owns all the objects we created above
    binaryen.Module.dispose(wasm_module);
}
