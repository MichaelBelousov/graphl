...
const GraphAreaWidget = @This();
// TODO: remove
/// deprecated
const ScrollContainerWidget = @This();

pub var defaults: Options = .{
    .name = "GraphAreaWidget",
...
pub fn render(src: std.builtin.SourceLocation, opts: Options) !*GraphAreaWidget {
    var ret = try dvui.currentWindow().arena().create(GraphAreaWidget);
    ret.* = GraphAreaWidget.init(src, &ret.si_slot, opts);
    try ret.install();
    return ret;
}
